...
def analyze_create_dialog(page, create_btn_text='Create'):
    """分析新建弹窗"""
    dialog_fields = []
    try:
        # 尝试点击新建按钮
        btn = page.locator(f'button:has-text("{create_btn_text}")').first
        if btn.count() > 0:
            btn.click()
            page.wait_for_timeout(1000)

            # 获取弹窗中的表单项
            form_items = page.locator('.ant-modal .ant-form-item').all()
            for item in form_items:
                try:
                    label = item.locator('.ant-form-item-label label').inner_text().strip()
                    if label:
                        required = item.locator('.ant-form-item-required').count() > 0
                        dialog_fields.append({'label': label, 'required': required})
                except:
                    pass

            # 关闭弹窗
            page.locator('.ant-modal-close, button:has-text("Cancel")').first.click()
            page.wait_for_timeout(500)
    except:
        pass
...
def analyze_page(page, url, name, index):
    ...
    # 截图
    screenshot = f'/tmp/cloudpods_network_{index}_{name.replace(" ", "_")}.png'
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # 分析各页面
        pages_to_analyze = [
            ('https://127.0.0.1/cloudregion', '区域', 1),
            ('https://127.0.0.1/zone', '可用区', 2),
            ('https://127.0.0.1/vpc-network', 'VPC互联', 3),
            ('https://127.0.0.1/globalvpc', '全局VPC', 4),
            ('https://127.0.0.1/vpc', 'VPC', 5),
            ('https://127.0.0.1/routetable', '路由表', 6),
            ('https://127.0.0.1/wire', '二层网络', 7),
            ('https://127.0.0.1/network', 'IP子网', 8),
        ]
        ...
        # 保存结果
        output_file = '/tmp/cloudpods_network_analysis.json'
