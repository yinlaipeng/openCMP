#!/usr/bin/env python3
"""
Cloudpods 负载均衡和 CDN 页面验证脚本
验证 openCMP 实现与 Cloudpods 设计一致性
"""
...
def analyze_page(page, url, name, index):
    ...
    # 截图
    screenshot = f'/tmp/cloudpods_lb_cdn_verify_{index}_{name.replace(" ", "_")}.png'
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # 保存结果
        output_file = '/tmp/cloudpods_lb_cdn_verify.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)

        print(f"\n{'='*60}")
        print(f"验证完成！结果保存: {output_file}")
