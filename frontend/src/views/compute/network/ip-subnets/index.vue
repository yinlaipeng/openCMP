  ...
  <div class="ip-subnets-container">
    ...
    <el-card class="filter-card">
      <el-form :inline="true" :model="filters" @submit.prevent="fetchData">
        ...
        <el-form-item label="VPC">
          <el-input v-model="filters.vpc_id" placeholder="VPC ID" clearable style="width: 150px" />
        </el-form-item>
    ...
    >
      <el-descriptions :column="2" border v-if="selectedSubnet">
        ...
        <el-descriptions-item label="自动分配">{{ selectedSubnet.is_auto_alloc ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="平台">
          <el-tag size="small" :type="getPlatformType(selectedSubnet.provider_type)">
            {{ getPlatformLabel(selectedSubnet.provider_type) }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="VPC">{{ selectedSubnet.vpc }}</el-descriptions-item>
        <el-descriptions-item label="可用区">{{ selectedSubnet.zone }}</el-descriptions-item>
