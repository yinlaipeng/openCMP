...
func main() {
	fmt.Println("START: 开始性能测试...")

	// 首先获取一个有效的令牌
	token, err := login("admin", "admin123")
	if err != nil {
		fmt.Printf("ERROR: 无法登录以进行性能测试: %v\n", err)
		return
	}
	fmt.Printf("SUCCESS: 成功获取令牌，开始性能测试\n")
...
func printPerformanceResult(result PerformanceResult) {
	...
	if result.ErrorCount > 0 {
		fmt.Printf("  WARNING: 发现 %d 个错误\n", result.ErrorCount)
	}

	if result.AvgTime > 2*time.Second {
		fmt.Printf("  WARNING: 平均响应时间较长\n")
	} else if result.AvgTime > 1*time.Second {
		fmt.Printf("  WARNING: 平均响应时间偏长\n")
	} else {
		fmt.Printf("  SUCCESS: 响应时间良好\n")
	}
