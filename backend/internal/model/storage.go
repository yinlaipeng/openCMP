...
type CloudSnapshot struct {
	...
	Name           string    `gorm:"size:200" json:"name"`
	DiskID         uint      `gorm:"index" json:"disk_id"`
	Size           int       `json:"size"` // GB
	Status         string    `gorm:"size:50" json:"status"`
	ProviderType   string    `gorm:"size:20" json:"provider_type"`
	CreatedAt      time.Time `json:"created_at"`
...
// TableName methods
func (CloudDisk) TableName() string     { return "storage_cloud_disks" }
func (CloudSnapshot) TableName() string { return "storage_cloud_snapshots" }