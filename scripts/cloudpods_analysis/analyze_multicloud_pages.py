#!/usr/bin/env python3
"""
Cloudpods 多云管理模块页面分析脚本
分析云账号、云用户组、代理、同步策略页面设计
"""
...
USERNAME = "admin"
PASSWORD = "admin@123"
OUTPUT_DIR = "/Users/aurora/Desktop/xtwork/git/openCMP/scripts/cloudpods_analysis/output_multicloud"
...
def analyze_page(page, page_name, page_path):
    ...
    try:
        ...
        analysis_result["elements"]["inputs"] = len(inputs)
        ...
        for item in form_items[:20]:
            try:
                ...
                if text.strip():
                    form_labels.append(text.strip())
            except:
                pass
        analysis_result["elements"]["form_labels"] = form_labels
...
def main():
    ...
    # 保存汇总结果
    summary_path = os.path.join(OUTPUT_DIR, "all_multicloud_pages_summary.json")
